...
if U>0
	...
	for i=1:D
		...
		if ~isempty(u_reg{i}) && ~isempty(u_reg{ji})
			...
			con = [con, P <= P_RU, B <= B_RU, ...
				T_0(i,:,:) + sum(sum(x(u_reg{i},:,:,:).* ...
					T4(u_reg{i},:,:,:),4),1) + sum(x(u_reg{ji},:,:,do_func).* ...
					T4(u_reg{ji},:,:,do_func),1) + TP_0(i,:,:) + ...
					repmat(sum(sum(x(u_reg{ji},:,:,no_func).* ...
					T4(u_reg{ji},:,:,no_func),3),1),1,1,S) <= T_RU/E* ...
