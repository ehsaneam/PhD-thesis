...
for m=1:rounds
	...
	splits(m,split7_1,1) = sum(sum(sum(results_info_do{m,4}(:,:,split7_1,:),4)));
	splits(m,split2,1) = sum(sum(sum(results_info_do{m,4}(:,:,split2,:),4)));
	splits(m,blocked_con,1) = sum(sum(sum(results_info_do{m,4}(:,:,blocked_con,:),4)));
	splits(m,split7_1,2) = sum(sum(sum(results_info_no{m,4}(:,:,split7_1,:),4)));
	splits(m,split2,2) = sum(sum(sum(results_info_no{m,4}(:,:,split2,:),4)));
	splits(m,blocked_con,2) = sum(sum(sum(results_info_no{m,4}(:,:,blocked_con,:),4)));
	
	funcs(m,do_func) = sum(sum(sum(results_info_do{m,4}(:,:,split7_1:split2,do_func))));
	funcs(m,no_func) = sum(sum(sum(results_info_do{m,4}(:,:,split7_1:split2,no_func))));
