...
for i=1:D
	j = (i==reg1)*reg2 + (i==reg2)*reg1;		% only for 2 regions, other region
	ji = (i==reg1)*reg21 + (i==reg2)*reg12;		% interfere signal
	ij = (i==reg1)*reg12 + (i==reg2)*reg21;		% signal cause interference
	...
	con = [con, T_0(i,:,split7_1) + sum(sum(x(u_reg{i},:,split7_1,:).* ...
				...
				T4(u_reg{ji},:,split7_1,:),4)) <= T_RU/E* ...
				sum(y(i,split7_1,:))];
	con = [con, T_0(i,:,split2) + sum(sum(x(u_reg{i},:,split2,:).* ...
				T4(u_reg{i},:,split2,:),4)) <= T_RU/E* ...
				sum(y(i,split2,:))];
	con = [con, sum(x(u_reg{ij},:,split2,no_func)) <= BIGM*w(i,:), ...
...
for i=1:D
	...
	P_0(i) = P_0(i) + sum(sum(sum(sum(P_res(u_reg{i},:,:,:))))) + ...
					  sum(sum(P_res(u_reg{j},:,split7_1,do_func)));
	B_0(i) = B_0(i) + sum(sum(sum(sum(B_res(u_reg{i},:,:,:))))) + ...
					  sum(sum(B_res(u_reg{j},:,split7_1,do_func)));
	T_0(i,:,split7_1) = T_0(i,:,split7_1) + ...
					  sum(sum(T_res(u_reg{i},:,split7_1,:),4)) + ...
					  sum(sum(T_res(u_reg{j},:,split7_1,:),4));
	T_0(i,:,split2) = T_0(i,:,split2) + ...
					  sum(sum(T_res(u_reg{i},:,split2,:),4));
end
