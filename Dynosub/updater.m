...
% 	u_reg_new{reg1} = find(ismember(u_round,u_reg{reg1}) & ismember(u_round,u_reg{reg1}));
	...
	for i=1:D													
		...
		util_T(i) = sum(TU_0(i) + sum(sum(sum(T_res(u_reg{i},:,:)))) + ...
		...
		T_0(i) = T_0(i) + sum(sum(sum(T_res(u_reg_fix{i},:,:)))) + ...
						  sum(T_res(u_reg_fix{ji},split7_1,do_func));

		TP_0(i) = TP_0(i) + sum(sum(T_res(u_reg_fix{ji},:,no_func)));
					  
		TU_0(i) = TU_0(i) + sum(sum(sum(val_x(u_reg_fix{i},:,:)))) + ...
						  sum(val_x(u_reg_fix{ji},split7_1,do_func));
	end
