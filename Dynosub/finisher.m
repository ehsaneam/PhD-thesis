...
if ~isempty(u_finish)
	P_tot_res = x_tot.*C3_tot.*P3_tot;
	B_tot_res = x_tot.*C3_tot.*B3_tot;
	...
	for i=1:D
		...
		T_0(i) = T_0(i) - sum(sum(sum(T_tot_res(fin_i,:,:)))) - ...
					  sum(T_tot_res(xfin_ji,split7_1,do_func));
		TP_0(i) = TP_0(i) - sum(sum(T_tot_res(xfin_ji,:,no_func)));
		TU_0(i) = TU_0(i) - sum(sum(sum(T_tot_res(fin_i,:,:)))) - ...
					  sum(T_tot_res(xfin_ji,split7_1,do_func));
	end
end